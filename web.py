...
def serve(data_dir: Path, host: str = "127.0.0.1", port: int = 8000, use_llm: bool = True, model: str | None = None) -> None:
    ...
    def get_agent(customer_id, conversation_id=None):
        session = str(conversation_id or "").strip()[:120]
        key = f"conversation:{session}" if session else str(customer_id or "anonymous")
        with lock:
            if key not in agents:
                agents[key] = StoreAgent(data_dir, customer_id=customer_id, use_llm=use_llm, model=model)
            elif customer_id is not None:
                # Customer identification may happen after the chat has already started.
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            try:
                payload=json.loads(self.rfile.read(int(self.headers.get('Content-Length','0')))); message=str(payload.get('message','')).strip(); raw=payload.get('customer_id'); customer_id=int(raw) if raw not in (None,'') else None; conversation_id=payload.get('conversation_id')
                if not message: raise ValueError('message is required')
                agent=get_agent(customer_id, conversation_id); agent.handoff_enabled=bool(payload.get('handoff',True))
