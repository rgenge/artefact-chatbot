...
4. The retrieved context goes to `gemini-3.1-flash-lite`, which writes short, warm
   open-ended answers.
...
question is routed to catalog, order, policy, exchange, checkout or handoff
without duplicating logic.

`examples/conversations.md` is generated by running the agent, not written by
hand — the transcripts there are real output.
...
- **Gemini 3.1 Flash Lite, the cheapest tier.** Here the LLM only writes over an
